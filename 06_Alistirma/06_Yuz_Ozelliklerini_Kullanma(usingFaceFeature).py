...
while True:
    ...
    if len(contours) is not None:
        ...
        cv2.circle(roi, extBottom, 5, (0, 0, 255), 2)

        cv2.line(roi, extLeft, extTop, (0, 0, 255), 2)
        cv2.line(roi, extTop, extRight, (0, 0, 255), 2)
        cv2.line(roi, extRight, extBottom, (0, 0, 255), 2)
        cv2.line(roi, extBottom, extLeft, (0, 0, 255), 2)

        a = math.sqrt(pow(extRight[0] - extTop[0], 2) + pow(extRight[1] - extTop[1], 2))
        b = math.sqrt(pow(extRight[0] - extBottom[0], 2) + pow(extRight[1] - extBottom[1], 2))
        c = math.sqrt(pow(extTop[0] - extBottom[0], 2) + pow(extTop[1] - extBottom[1], 2))
